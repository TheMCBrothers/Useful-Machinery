...
public abstract class AbstractMachineFluidScreen<T extends MachineMenu> extends AbstractMachineScreen<T> {
    ...
    public AbstractMachineFluidScreen(T screenContainer, Inventory inv, Component titleIn, int tankCapacity) {
        this(tankCapacity, screenContainer, inv, titleIn);
    }

    private AbstractMachineFluidScreen(int capacity, T screenContainer, Inventory inv, Component titleIn) {
    ...
    public List<Component> getTooltip(FluidStack fluidStack) {
        ...
        try {
            ...
            tooltip.add(TextUtils.fluidWithMaxName(fluidStack.getDisplayName().getString(), milliBuckets, capacity));
        } catch (RuntimeException e) {
            UsefulMachinery.LOGGER.error("Failed to get tooltip for fluid: " + e);
        }
