...
public class MachineryBlocks {
    private final static Item.Properties ITEM_PROPS = new Item.Properties().tab(UsefulMachinery.GROUP);
    private final static Function<? super Block, MachineBlockItem> MACHINE_BLOCK_ITEM = block -> new MachineBlockItem(block, ITEM_PROPS);
    ...
    public static final ItemObject<CreativePowerCellBlock> CREATIVE_POWER_CELL = BLOCKS.register("creative_power_cell", () -> new CreativePowerCellBlock(Block.Properties.of(Material.DECORATION).strength(.5f).sound(SoundType.METAL).noDrops()), block -> new CreativePowerCellItem(block, ITEM_PROPS));
